defmodule Claper.Interactions do
  ...
  def get_interactions_at_position(
    ...
    with polls <- Polls.list_polls_at_position(presentation_file_id, position),
         ...
         word_clouds <- WordClouds.list_word_clouds_at_position(presentation_file_id, position),
         open_ended <- OpenEnded.list_open_ended_at_position(presentation_file_id, position) do
      interactions =
        (polls ++ forms ++ embeds ++ quizzes ++ word_clouds ++ open_ended)
  ...
  defp do_move_interaction(event_uuid, %WordClouds.WordCloud{} = word_cloud, to),
    do: WordClouds.update_word_cloud(event_uuid, word_cloud, %{position: to, enabled: false})

  defp do_move_interaction(event_uuid, %OpenEnded.OpenEnded{} = open_ended, to),
    do: OpenEnded.update_open_ended(event_uuid, open_ended, %{position: to, enabled: false})
  ...
  def enable_interaction(interaction) do
    ...
    |> Ecto.Multi.run(:disable_open_ended, fn _repo, _ ->
      {count, _} = OpenEnded.disable_all(interaction.presentation_file_id, interaction.position)
      {:ok, count}
    end)
  ...
  defp set_enabled(%OpenEnded.OpenEnded{} = interaction) do
    OpenEnded.set_enabled(interaction.id)
  end
  ...
  def disable_interaction(%OpenEnded.OpenEnded{} = interaction) do
    OpenEnded.set_disabled(interaction.id)
  end
end
