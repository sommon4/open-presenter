defmodule Claper.Events do
         ...
         |> Ecto.Multi.run(:to_event, fn _repo, _changes ->
           {:ok,
            get_user_event!(user_id, to_event_uuid,
              presentation_file: [:polls, :forms, :embeds, :word_clouds, :open_ended]
            )}
         end)
            ...
            |> Enum.each(fn word_cloud ->
              if word_cloud.position < to_event.presentation_file.length do
                Claper.WordClouds.create_word_cloud(%{
                  ...
                  enabled: false,
                  max_answers: word_cloud.max_answers,
                  max_characters: word_cloud.max_characters,
                  show_results: word_cloud.show_results,
                  merge_case: word_cloud.merge_case,
                  moderation_enabled: word_cloud.moderation_enabled,
                  profanity_filter_enabled: word_cloud.profanity_filter_enabled,
                  presentation_file_id: to_event.presentation_file.id
                })
              end
            end)}
         end)
         |> Ecto.Multi.run(:open_ended, fn _repo, %{from_event: from_event, to_event: to_event} ->
           {:ok,
            from_event.presentation_file.open_ended
            |> Enum.each(fn item ->
              if item.position < to_event.presentation_file.length do
                Claper.OpenEnded.create_open_ended(%{
                  title: item.title,
                  position: item.position,
                  enabled: false,
                  max_characters: item.max_characters,
                  multiple_responses: item.multiple_responses,
                  voting_enabled: item.voting_enabled,
                  auto_scroll: item.auto_scroll,
                  show_results: item.show_results,
                  moderation_enabled: item.moderation_enabled,
                  presentation_file_id: to_event.presentation_file.id
                })
              end
            end)}
  ...
  def duplicate_event(user_id, event_uuid) do
    ...
    multi =
      ...
      |> Ecto.Multi.run(:open_ended, fn _repo, changes ->
        duplicate_open_ended(original, changes)
      end)
  ...
  defp duplicate_open_ended(original, changes) do
    case get_in(original.presentation_file.open_ended) do
      items when is_list(items) ->
        items =
          for item <- items do
            ...
            {:ok, item} = Claper.OpenEnded.create_open_ended(attrs)
            item
          end
