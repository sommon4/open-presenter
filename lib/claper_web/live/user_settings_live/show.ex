defmodule ClaperWeb.UserSettingsLive.Show do
  ...
  def mount(_params, session, socket) do
    ...
    {:ok,
     ...
     |> assign(:office_tokens, Claper.Office.list_tokens(socket.assigns.current_user))
     |> assign(:new_office_token, nil)}
  end
  ...
  @impl true
  def handle_event("generate-office-token", params, socket) do
    name = String.trim(params["name"] || "")
    name = if name == "", do: gettext("PowerPoint"), else: name

    scopes =
      if params["write"] == "true",
        do: Claper.Office.Token.scopes(),
        else: Claper.Office.Token.default_scopes()

    case Claper.Office.create_token(socket.assigns.current_user, name, scopes) do
      {:ok, plain, _token} ->
        {:noreply,
         socket
         |> assign(:new_office_token, plain)
         |> assign(:office_tokens, Claper.Office.list_tokens(socket.assigns.current_user))}

      {:error, _changeset} ->
        {:noreply, socket |> put_flash(:error, gettext("Could not create the token"))}
    end
  end

  @impl true
  def handle_event("revoke-office-token", %{"id" => id}, socket) do
    _ = Claper.Office.delete_token(socket.assigns.current_user, id)
  ...
  @impl true
  def handle_event("dismiss-office-token", _params, socket) do
    {:noreply, assign(socket, :new_office_token, nil)}
  end
