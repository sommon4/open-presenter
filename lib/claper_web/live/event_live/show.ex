defmodule ClaperWeb.EventLive.Show do
  ...
  defp init(socket, event, false) do
    ...
    socket =
      ...
      |> assign(:word_cloud_responses, [])
      |> assign(:word_cloud_error, nil)
      |> assign(:open_ended_responses, [])
      |> assign(:open_ended_voted_ids, [])
      |> assign(:open_ended_error, nil)
  ...
  @impl true
  def handle_info({:word_cloud_updated, %Claper.WordClouds.WordCloud{}}, socket) do
    {:noreply, refresh_current_interaction(socket, true)}
  end

  @impl true
  def handle_info({:word_cloud_deleted, %Claper.WordClouds.WordCloud{}}, socket) do
    {:noreply, refresh_current_interaction(socket, true)}
  end

  @impl true
  def handle_info({:open_ended_updated, %Claper.OpenEnded.OpenEnded{}}, socket) do
    {:noreply, refresh_current_interaction(socket, true)}
  end

  @impl true
  def handle_info({:open_ended_deleted, %Claper.OpenEnded.OpenEnded{}}, socket) do
    {:noreply, refresh_current_interaction(socket, true)}
  end
  ...
  def handle_event(
    ...
    case WordClouds.submit_response(
           attendee_identity(socket),
           socket.assigns.event.uuid,
           word_cloud,
           word
         ) do
      {:ok, _response, word_cloud} ->
        {:noreply,
         socket
         |> assign(:current_interaction, word_cloud)
         |> assign(:word_cloud_error, nil)
         |> get_current_word_cloud_responses(word_cloud.id)}

      {:error, reason} ->
        {:noreply, socket |> assign(:word_cloud_error, word_cloud_error(reason))}
    end
  end
  ...
  def handle_event(
        "submit-open-ended",
        %{"text" => text},
        %{assigns: %{current_interaction: %OpenEnded.OpenEnded{} = open_ended}} = socket
      ) do
    case OpenEnded.submit_response(
           attendee_identity(socket),
           socket.assigns.event.uuid,
           open_ended,
           text
         ) do
      {:ok, _response, open_ended} ->
        {:noreply,
         socket
         |> assign(:current_interaction, open_ended)
         |> assign(:open_ended_error, nil)
         |> get_current_open_ended_responses(open_ended.id)}

      {:error, reason} ->
        {:noreply, socket |> assign(:open_ended_error, open_ended_error(reason))}
    end
  end

  @impl true
  def handle_event("submit-open-ended", _params, socket), do: {:noreply, socket}
  ...
  def handle_event(
    ...
    case OpenEnded.toggle_vote(
           attendee_identity(socket),
           socket.assigns.event.uuid,
           open_ended,
           String.to_integer(id)
         ) do
      {:ok, _action, open_ended} ->
        {:noreply,
         socket
         |> assign(:current_interaction, open_ended)
         |> get_current_open_ended_responses(open_ended.id)}
  ...
  @impl true
  def handle_event("vote-open-ended", _params, socket), do: {:noreply, socket}
  ...
  defp load_current_interaction(
         socket,
         %WordClouds.WordCloud{} = interaction,
         same_interaction
       ) do
    socket
    |> assign(:current_interaction, interaction)
    |> then(fn s -> if same_interaction, do: s, else: assign(s, :word_cloud_error, nil) end)
    |> get_current_word_cloud_responses(interaction.id)
  end

  defp load_current_interaction(
         socket,
         %OpenEnded.OpenEnded{} = interaction,
         same_interaction
       ) do
    socket
    |> assign(:current_interaction, interaction)
    |> then(fn s -> if same_interaction, do: s, else: assign(s, :open_ended_error, nil) end)
    |> get_current_open_ended_responses(interaction.id)
  end
  ...
  defp attendee_identity(%{assigns: %{current_user: %{id: user_id}}}) when is_integer(user_id),
    do: user_id

  defp attendee_identity(%{assigns: %{attendee_identifier: attendee_identifier}}),
    do: attendee_identifier

  defp get_current_word_cloud_responses(socket, word_cloud_id) do
    assign(
      socket,
      :word_cloud_responses,
      WordClouds.list_responses_for(attendee_identity(socket), word_cloud_id)
    )
  end

  defp get_current_open_ended_responses(socket, open_ended_id) do
    identity = attendee_identity(socket)

    socket
    |> assign(:open_ended_responses, OpenEnded.list_responses_for(identity, open_ended_id))
    |> assign(:open_ended_voted_ids, OpenEnded.voted_response_ids(identity, open_ended_id))
  end

  defp open_ended_error(:empty), do: gettext("Please type a response")
  defp open_ended_error(:too_long), do: gettext("Your response is too long")
  defp open_ended_error(:limit_reached), do: gettext("You have already responded")
  defp open_ended_error(:rate_limited), do: gettext("Too many responses, please slow down")
  defp open_ended_error(:full), do: gettext("This question is full")
  defp open_ended_error(:disabled), do: gettext("This question is closed")
  defp open_ended_error(_), do: gettext("Your response could not be saved")

  defp word_cloud_error(:empty), do: gettext("Please type an answer")
  defp word_cloud_error(:too_long), do: gettext("Your answer is too long")
  defp word_cloud_error(:duplicate), do: gettext("You already sent this answer")
  defp word_cloud_error(:limit_reached), do: gettext("You have sent all your answers")
  defp word_cloud_error(:rate_limited), do: gettext("Too many answers, please slow down")
  defp word_cloud_error(:profanity), do: gettext("This answer is not allowed")
  defp word_cloud_error(:disabled), do: gettext("This word cloud is closed")
  defp word_cloud_error(_), do: gettext("Your answer could not be saved")
