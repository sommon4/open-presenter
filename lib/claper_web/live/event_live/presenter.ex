defmodule ClaperWeb.EventLive.Presenter do
  ...
  alias Claper.OpenEnded.OpenEnded
  ...
  def handle_info({:word_cloud_updated, word_cloud}, socket) do
    if word_cloud.enabled do
      {:noreply, socket |> assign(:current_word_cloud, word_cloud)}
    else
      {:noreply, socket |> assign(:current_word_cloud, nil)}
    end
  end
  ...
  @impl true
  def handle_info({:open_ended_updated, open_ended}, socket) do
    if open_ended.enabled do
      {:noreply, socket |> assign(:current_open_ended, open_ended)}
    else
      {:noreply, socket |> assign(:current_open_ended, nil)}
    end
  end

  @impl true
  def handle_info({:open_ended_deleted, _open_ended}, socket) do
    {:noreply, socket |> assign(:current_open_ended, nil)}
  end
  ...
  @impl true
  def handle_info(
        {:current_interaction, %OpenEnded{} = interaction},
        socket
      ) do
    {:noreply,
     socket
     |> assign(:current_open_ended, interaction)
     |> assign(:current_word_cloud, nil)
     |> assign(:current_poll, nil)
     |> assign(:current_embed, nil)
     |> assign(:current_form, nil)
  ...
  defp open_ended_at_position(%{assigns: %{event: event, state: state}} = socket) do
    socket
    |> assign(
      :current_open_ended,
      Claper.OpenEnded.get_open_ended_current_position(event.presentation_file.id, state.position)
    )
  end
