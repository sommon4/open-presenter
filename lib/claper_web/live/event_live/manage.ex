defmodule ClaperWeb.EventLive.Manage do
  ...
  defp get_interaction_for_event("open_ended", id, socket),
    do: OpenEnded.get_open_ended_for_event(id, event_id(socket))
  ...
  @impl true
  def handle_info({:open_ended_created, open_ended}, socket) do
    {:noreply, socket |> interactions_at_position(open_ended.position)}
  end

  @impl true
  def handle_info({:open_ended_updated, _open_ended}, socket) do
    {:noreply, socket |> interactions_at_position(socket.assigns.state.position)}
  end

  @impl true
  def handle_info({:open_ended_deleted, open_ended}, socket) do
    {:noreply, socket |> interactions_at_position(open_ended.position)}
  end
  ...
  def handle_event(
      ...
      when is_integer(id) and is_integer(to) and
             type in ["poll", "form", "embed", "quiz", "word_cloud", "open_ended"] do
  ...
  @impl true
  def handle_event("word_cloud-set-active", %{"id" => id}, socket) do
    case WordClouds.get_word_cloud_for_event(id, event_id(socket)) do
      ...
      word_cloud ->
        with :ok <- Claper.Interactions.enable_interaction(word_cloud) do
          word_cloud = %{word_cloud | enabled: true}
  ...
  def handle_event("word_cloud-set-inactive", %{"id" => id}, socket) do
    case WordClouds.get_word_cloud_for_event(id, event_id(socket)) do
      nil ->
        {:noreply, socket}
  ...
  @impl true
  def handle_event("open_ended-set-active", %{"id" => id}, socket) do
    case OpenEnded.get_open_ended_for_event(id, event_id(socket)) do
      nil ->
        {:noreply, socket}

      open_ended ->
        with :ok <- Claper.Interactions.enable_interaction(open_ended) do
          open_ended = %{open_ended | enabled: true}

          Phoenix.PubSub.broadcast(
            Claper.PubSub,
            "event:#{socket.assigns.event.uuid}",
            {:current_interaction, open_ended}
          )
  ...
  def handle_event("open_ended-set-inactive", %{"id" => id}, socket) do
    case OpenEnded.get_open_ended_for_event(id, event_id(socket)) do
      nil ->
        {:noreply, socket}

      open_ended ->
        with {:ok, _} <- Claper.Interactions.disable_interaction(open_ended) do
          Phoenix.PubSub.broadcast(
            Claper.PubSub,
            "event:#{socket.assigns.event.uuid}",
            {:current_interaction, nil}
  ...
  def handle_event(
        "checked",
        %{"key" => key, "value" => value},
        %{assigns: %{current_interaction: %OpenEnded.OpenEnded{} = interaction}} = socket
      )
      when key in ["open_ended_show_results", "open_ended_voting", "open_ended_auto_scroll"] do
    field =
      case key do
        "open_ended_show_results" -> :show_results
        "open_ended_voting" -> :voting_enabled
        "open_ended_auto_scroll" -> :auto_scroll
      end

    {:ok, new_interaction} =
      OpenEnded.update_open_ended(socket.assigns.event.uuid, interaction, %{field => value})

    {:noreply, socket |> assign(:current_interaction, new_interaction)}
  end

  @impl true
  def handle_event("open-ended-moderate", %{"id" => id, "status" => status}, socket)
      when status in ["approved", "rejected"] do
    case OpenEnded.get_response_for_event(id, event_id(socket)) do
      nil ->
        {:noreply, socket}

      response ->
        {:ok, _} = OpenEnded.moderate_response(socket.assigns.event.uuid, response, status)
        {:noreply, socket}
    end
  end

  @impl true
  def handle_event("open-ended-delete-response", %{"id" => id}, socket) do
    case OpenEnded.get_response_for_event(id, event_id(socket)) do
      nil ->
        {:noreply, socket}

      response ->
        {:ok, _} = OpenEnded.delete_response(socket.assigns.event.uuid, response)
        {:noreply, socket}
    end
  end
  ...
  def handle_event("open-ended-reset", %{"id" => id}, socket) do
    case OpenEnded.get_open_ended_for_event(id, event_id(socket)) do
      ...
      open_ended ->
        {:ok, _} = OpenEnded.delete_all_responses(socket.assigns.event.uuid, open_ended)
        {:noreply, socket}
    end
  end
  ...
  def handle_event(
        ...
        %{assigns: %{current_interaction: %WordClouds.WordCloud{} = interaction}} = socket
      ) do
    {:ok, new_interaction} =
      WordClouds.update_word_cloud(socket.assigns.event.uuid, interaction, %{show_results: value})

    {:noreply, socket |> assign(:current_interaction, new_interaction)}
  end

  @impl true
  def handle_event("word-cloud-moderate", %{"id" => id, "status" => status}, socket)
      when status in ["approved", "rejected"] do
    case WordClouds.get_response_for_event(id, event_id(socket)) do
      nil ->
        {:noreply, socket}

      response ->
        {:ok, _} = WordClouds.moderate_response(socket.assigns.event.uuid, response, status)
        {:noreply, socket}
    end
  end
  ...
  def handle_event(
    ...
    case WordClouds.get_word_cloud_for_event(id, event_id(socket)) do
      ...
      word_cloud ->
        key = Claper.WordClouds.Normalizer.key(text, word_cloud.merge_case)
        {:ok, _} = WordClouds.delete_word(socket.assigns.event.uuid, word_cloud, key)
        {:noreply, socket}
    end
  end

  @impl true
  def handle_event("word-cloud-reset", %{"id" => id}, socket) do
    case WordClouds.get_word_cloud_for_event(id, event_id(socket)) do
      nil ->
        {:noreply, socket}

      word_cloud ->
        {:ok, _} = WordClouds.delete_all_responses(socket.assigns.event.uuid, word_cloud)
        {:noreply, socket}
    end
  end
  ...
  @impl true
  def handle_event("delete-open_ended", %{"id" => id}, socket) do
    case OpenEnded.get_open_ended_for_event(id, event_id(socket)) do
      nil ->
        {:noreply, socket}

      open_ended ->
        {:ok, _} = OpenEnded.delete_open_ended(socket.assigns.event.uuid, open_ended)
        {:noreply, socket}
    end
  end

  @impl true
  def handle_event("delete-word_cloud", %{"id" => id}, socket) do
    case WordClouds.get_word_cloud_for_event(id, event_id(socket)) do
      nil ->
        {:noreply, socket}
  ...
  defp apply_action(socket, :add_open_ended, _params) do
    socket
    |> assign(:create, "open_ended")
    |> assign(:interaction_modal, true)
    |> assign(:create_action, :new)
    |> assign(:open_ended, %OpenEnded.OpenEnded{
      presentation_file_id: socket.assigns.event.presentation_file.id
    })
  end

  defp apply_action(socket, :edit_open_ended, %{"id" => id}) do
    case OpenEnded.get_open_ended_for_event(id, event_id(socket)) do
      nil ->
        socket
        |> put_flash(:error, gettext("Resource not found"))
