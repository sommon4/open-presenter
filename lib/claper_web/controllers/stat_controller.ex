defmodule ClaperWeb.StatController do
  ...
  @doc """
  Exports the aggregated answers of a word cloud as a CSV file.
  Requires user to be either an event leader or the event owner.
  """
  def export_word_cloud(%{assigns: %{current_user: current_user}} = conn, %{
        "word_cloud_id" => word_cloud_id
      }) do
    with word_cloud <- WordClouds.get_word_cloud!(word_cloud_id),
         presentation_file <-
           Presentations.get_presentation_file!(word_cloud.presentation_file_id, [:event]),
         event <- presentation_file.event,
         :ok <- authorize_event_access(current_user, event) do
      headers = ["Answer", "Merged as", "Count"]
      content = WordClouds.export_rows(word_cloud.id)

      export_as_csv(conn, headers, content, "word-cloud-#{sanitize(word_cloud.title)}")
    else
      :unauthorized -> send_resp(conn, 403, "Forbidden")
    end
  end
  ...
  @doc """
  Exports the responses of an open ended question as a CSV file.
  Requires user to be either an event leader or the event owner.
  """
  def export_open_ended(%{assigns: %{current_user: current_user}} = conn, %{
        "open_ended_id" => open_ended_id
      }) do
    with open_ended <- OpenEnded.get_open_ended!(open_ended_id),
         presentation_file <-
           Presentations.get_presentation_file!(open_ended.presentation_file_id, [:event]),
         event <- presentation_file.event,
         :ok <- authorize_event_access(current_user, event) do
      headers = ["Response", "Votes", "Status", "Sent at (UTC)"]
      content = OpenEnded.export_rows(open_ended.id)

      export_as_csv(conn, headers, content, "open-ended-#{sanitize(open_ended.title)}")
    else
      :unauthorized -> send_resp(conn, 403, "Forbidden")
    end
  end
