...
config :claper,
       :word_cloud_profanity_words,
       get_var_from_path_or_env(config_dir, "WORD_CLOUD_PROFANITY_WORDS", "")
       |> String.split(",", trim: true)
       |> Enum.map(&String.trim/1)

config :claper, :office_addin_id, get_var_from_path_or_env(config_dir, "OFFICE_ADDIN_ID", nil)
